/* ------------------------------------------------*
   digraphsEnumChordlessCircuitsInOutPiping.cpp
   ...
   compile with
   g++ -Wall -O4 digraphsEnumChordlessCircuitsInOutPiping.cpp -o enumChordlessCircuitsInOutPiping -lagrum 
...
// creating empty graphs
 ...
 // readin the graph data from ifstream
 // assert(argc == 2);
 // ifstream in(argv[1]);
 // assert(in);
 //int order = 0;
 ...
 while (cin.good()) {
   // cout << an1 << " " << an2 << endl;
   n1 = atoi(an1);
   if (!g1.existsNode(n1)){g1.insertNode(n1);}
   n2 = atoi(an2);
   if (!g1.existsNode(n2)){g1.insertNode(n2);}
   NodeId initialNode = n1;
   NodeId terminalNode = n2;
 ...
 // cout << "Execution time: " << cdiff << " sec." << endl;
 // cout << "with C++/Agrum, R Bisdorff, July 2011 " << endl;
 ...
 cout << "]" << endl;
 // out.close();
 // cout << "\n\nSample counts of chordless circuits sizes:";
 //   for (int ii = 0; ii < order; ii++) printf("\n%i  %9u", ii, x[ii]);
 //   //for (int ii = 0; ii < order; ii++) cout << endl << ii << "\t" << x[ii];
 // printf("\n\n");
 // cout << endl << endl; 

 // printf("\n\nVisited chordless P2s :");
 // clog << visistedArcs.size() << ", " << visitedArcs.toString() << endl;
