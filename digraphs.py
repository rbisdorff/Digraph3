...
class Digraph(object):
    ...
    def showAttributes(self):
        ...
        """
        reprString = '*------- Attributes of Digraph ------*\n'
        reprString += 'Instance name : %s\n' % self.name
        reprString += 'Instance class: %s\n' % self.__class__.__name__
        atts = list(self.__dict__.keys())
        atts.sort()
        reprString += 'Attributes    : %s\n' % atts       
        print(reprString)        
...
##            else:
    ...
    def _computeRankingByLastChoosing(self,CoDual=False,CppAgrum=False,Debug=False):
        ...
        while len(remainingActions) > 1 and worstChoice[1] != []:
            ...
            worstChoiceCandidates.sort(reverse=True,key=itemgetter(0))
            try:
        ...
        if (bestChoice[1] == [] and worstChoice[1] == []):
            ...
            for ch in currGcd.goodChoices:
                ...
                if len(ck1) > 0:
                    ...
                    bestChoiceCandidates.append((self.valuationdomain['max'],k1))
            bestChoiceCandidates.sort(reverse=True, key=itemgetter(0))
            try:
                bestChoice = bestChoiceCandidates[0]
            except:
                #print 'Error: no best choice in currGcd!'
...
##            else:
    ...
    def _optimalRankingByChoosing(self,Odd=True,CoDual=False,Comments=False,Debug=False,Limited=None):
        ...
        if i==0:
            ...
            rankings.sort(reverse=True,key=itemgetter(0))
...
##                    rankingRelation[y][x] = self.omin( [rankingRelation[y][x],abs(relation[x][y])] )
    ...
    def showRankingByLastChoosing(self,rankingByLastChoosing=None,Debug=None):
        ...
        """
        if rankingByLastChoosing is None:
            try:
                rankingByLastChoosing = self.rankingByLastChoosing['result']
            except:
    ...
    def showRankingByBestChoosing(self,rankingByBestChoosing=None):
        ...
        """
        if rankingByBestChoosing is None:
            try:
                rankingByBestChoosing = self.rankingByBestChoosing['result']
            except:
    ...
    def computeBipolarCorrelation(self, other, MedianCut=False,
                                  filterRelation=None, Debug=False):
        """
    ...
    def computeDynamicProgrammingStages(self,source,sink,Debug=False):
        """
        ...
        if len(stages[0]) != 1  and len(stages[-1]) != 1:
            print('Error: the given digraph is not a valid dynamic programming digraph')
        else:
            return stages
    ...
    def convertValuation2Integer(self,InSite=True,Comments=False):
        """
        ...
        *Parameters*:
        ...
        n = len(actions)
        try:
            Hiv = self.valuationdomain['hasIntegerValuation']
        except:
            self.valuationdomain['hasIntegerValuation'] = False
        if not self.valuationdomain['hasIntegerValuation']:
            t = []
            for i in range(n):
                x = actions[i]
                for j in range(i+1,n):
                    y = actions[j]
                    t.append(Fraction(self.relation[x][y]).as_integer_ratio()[1])
            # requires Python3.9+
            valint = lcm(*t) 
            # before Python3.9 uncomment below !
            # from functools import reduce
            # valint = reduce(lambda x, y: lcm(x,y),t,t[0])
            if Comments:
                print('Leat common multiple of denominators is ',valint)
            if InSite:
                if Comments:
                    print('Recoding valuation to [%d,%d]' % (-valint,valint) )
                self.recodeValuation(-valint,valint,ndigits=0)
                self.valuationdomain['hasIntegerValuation'] = True
            else:
                if Comments:
                    print('renders the recode sel.relation to [%d,%d]' % (-valint,valint) )
                integerRelation = {}
                for x in actions:
                    integerRelation[x] = {}
                    for y in actions:
                        integerRelation[x][y] = Decimal('%.0f' % (self.relation[x][y] * Decimal(str(valint)) ))
                return integerRelation
        else:
            print('The relation is already integer-valued')
    ...
    def isIntegerValued(self, Debug=False):
        """
        ...
        for x in actions:
            for y in actions:
                ratio = relation[x][y].as_integer_ratio()
                if Debug:
                    print(x,y,relation[x][y],ratio)
                if ratio[1] != 1:
                    IntegerValued = False
    ...
    def isOutrankingDigraph(self,Comments=True,Debug=False):
        """
        ...
        """
        from copy import deepcopy
        #normalizing the valuation domain
        g = deepcopy(self)
        g.recodeValuation()
        if Debug:
        ...
        Med = g.valuationdomain['med']
        # if Med != Decimal('0.0'):
        #     print("Error: the digraph's valuation  must be bipolar !!")
        ...
        for i in range(n):
            ...
            for j in range(i+1,n):
                ...
                if (relation[x][y] + relation[y][x]) < Med:
                    IsOutrankingDigraph = False
                    if Comments or Debug:
                        print('x,y,relation[x][y],relation[y][x]',\
                               x, y, relation[x][y], relation[y][x])
                        print('Not a valid outranking valuation') 
 
        return IsOutrankingDigraph

    def isStrictOutrankingDigraph(self,Comments=True,Debug=False):
        """
        ...
        IsStrictOutrankingDigraph = True
        ...
        for i in range(n):
            ...
            for j in range(i+1,n):
                ...
                if relation[x][y] + relation[y][x] > Med:
                    IsStrictOutrankingDigraph = False
                    if Comments or Debug:
                        print('x,y,relation[x][y],relation[y][x]',\
                               x, y, relation[x][y], relation[y][x])
                        print('Not a valid strict outranking valuation')  
 
        return IsStrictOutrankingDigraph

    def isAsymmetricIndeterminate(self, Debug=False):
        """
        ...
        AsymmetricIndeterminate = False
        for i in range(n):
            x = listActions[i]
            for j in range(i+1,n):
                y = listActions[j]
                if (relation[x][y] == Med and relation[y][x] != Med) or\
                   (relation[x][y] != Med and relation[y][x] == Med):
                    AsymmetricIndeterminate = True
                    if Debug:
                        print('x,y,relation[x][y],relation[y][x]',\
                              x, y, relation[x][y], relation[y][x])
        return AsymmetricIndeterminate
    ...
    def showHTMLRelationHeatmap(self,actionsList=None,
                            rankingRule='NetFlows',
                            colorLevels=7,
                            tableTitle='Relation Heatmap',
                            relationName='r(x S y)',
                            ndigits=2,
                            fromIndex=None,
                            toIndex=None,
        ...
        if htmlFileName == None:
            ...
            fileName = getcwd()+'/'+htmlFileName
        fo = open(fileName,'w')
        fo.write(self._htmlRelationHeatmap(actionsSubset=actionsList,
                                      rankingRule=rankingRule,
    ...
    def _htmlRelationHeatmap(self,tableTitle='Relation Heatmap',
                          relationName='r(x R y)',
                          actionsSubset= None,
                          rankingRule='Copeland',
                          colorLevels=7,
        ...
        Max = self.valuationdomain['max']
        # set colo palette
        brewerRdYlGn9Colors = [(Decimal('0.125'),'"#D53E4F"'),
                               (Decimal('0.25'),'"#F46D43"'),
                               (Decimal('0.375'),'"#FDAE61"'),
                               (Decimal('0.499999'),'"#FEE08B"'),
                               (Decimal('0.5'),'"#FFFFFF"'),
                               (Decimal('0.625'),'"#D9EF8B"'),
                               (Decimal('0.75'),'"#A6D96A"'),
                               (Decimal('0.875'),'"#65BD63"'),
                               (Decimal('1.000'),'"#1A9850"')]
        # _brewerRdYlGn7Colors = [
        ...
        #                        ]
        brewerRdYlGn7Colors = [
                               (Decimal('0.166666'),'"#F46D43"'),
                               (Decimal('0.333333'),'"#FDAE61"'),
                               (Decimal('0.499999'),'"#FEE08B"'),
                               (Decimal('0.5'),'"#FFFFFF"'),
                               (Decimal('0.666666'),'"#D9EF8B"'),
                               (Decimal('0.833333'),'"#A6D96A"'),
                               (Decimal('1.0000'),'"#65BD63"')
                               ]
        brewerRdYlGn5Colors = [
                               (Decimal('0.25'),'"#FDAE61"'),
        ...
        if Debug:
            print(actionsList)
        # compute relation characteristic colors
        amplitude = Max - Min
        relationColor = {}
        for x in actions:
            relationColor[x] = {}
            for y in actions:
                rxy = self.relation[x][y] - Min
                xych = rxy/amplitude
                for cl in range(nc):
                    if xych <= colorPalette[cl][0]:
                        relationColor[x][y] = colorPalette[cl][1]
                        break
                    else:
                        relationColor[x][y] = colorPalette[-1][1]
                        
                if Debug:
                    print(x,y,rxy,xych,relationColor[x][y])
        # construct html text
        ...
        html += '</style>\n'
        html += '</head>\n<body>\n'
        html += '<h1>%s</h1>' % tableTitle
        if rankingRule is not None:
            html += '<h2>Ranking rule: %s</h2>' % rankingRule
        html += '<table border="0">\n'
        html += '<tr bgcolor="#9acd32"><th>%s</th>\n' % relationName
        for x in actionsList:
            html += '<th bgcolor="#FFF79B">%s</th>\n' % (x[0])
        html += '</tr>\n'
        for x in actionsList:
            xKey = x[1]
            html += '<tr>'
            ...
            for y in actionsList:
                yKey = y[1]
                if Debug:
                    print(x,y,self.relation[xKey][yKey],relationColor[xKey][yKey])
                if self.relation[xKey][yKey] != Med:
                    formatString = '<td bgcolor=%s align="right">%%+ .%df</td>' \
                                                % (relationColor[xKey][yKey],ndigits)
                else:
                    formatString = '<td align="right">%%+ .%df</td>' % (ndigits)
                html += formatString % (self.relation[xKey][yKey])
                    
            html += '</tr>'
        html += '</table>\n'
        # legend
        html += '<table style="background-color:%s;">\n' % (backGroundColor)
        html += '<tr bgcolor=%s><td>Valuation domain</td>' % (backGroundColor)
        formatString = '<td>[%% .%df;</td><td>%% .%df]</td></tr>' \
                                            % (ndigits,ndigits)
        html += formatString % (Min,Max)
        html += '</table>\n'
        html += '<b>Color legend: </b>\n'
        html += '<table style="background-color:%s;" border="1">\n' % (backGroundColor)
        html += '<tr bgcolor=%s><td>Levels</td>' % (backGroundColor)
        #mnc = (nc+1)//2
        for col in range(0,nc):
            # if col == (mnc-1):
            #     html += '<td>&nbsp;%.2f</td>' % Decimal('0')
            if col == 0:
                formatString = '<td bgcolor=%%s>[%%+ .%df;%%+ .%df]</td>' \
                                             % (ndigits,ndigits)
                html += formatString  \
                % ( colorPalette[col][1], (Decimal('-1.0')*Max),
                  ( ((colorPalette[col][0]*Decimal('2'))-Decimal('1'))*Max) )
            else:
                formatString = '<td bgcolor=%%s>&nbsp;%%+ .%df]</td>' % ndigits
                html += formatString \
                % ( colorPalette[col][1],
                  ( ((colorPalette[col][0]*Decimal('2'))-Decimal('1'))*Max) )
        html += '</tr>\n'
    ...
    def showHTMLRelationTable(self,actionsList=None,
        ...
        if htmlFileName == None:
            ...
            from os import getcwd
            fileName = getcwd()+'/'+htmlFileName
        #fileName = tempDir+'/relationTable.html'
        fo = open(fileName,'w')
...
##            actionsList.sort()
    ...
    def exportPrincipalImage(self, 
        ...
        if Comments:
            print('*----  export 3dplot of format %s -----' % (pictureFormat))
        if tempDir is None:
            tempDir = '.'
        import os,time
        if plotFileName is None:
            plotFileName = "%s/%s" % (tempDir,self.name)
        else:
            plotFileName = "%s/%s" % (tempDir,plotFileName)
        if Colwise:
            plotFileName += "_Colwise"
        else:
            plotFileName += "_Rowwise"
        self.saveCSV('%s/exportTemp' % tempDir,Diagonal=True)
        if Colwise:
        ...
        if pictureFormat is None:
            # no principal image is required
            fo.close()
        else:
            fo.write("valprop = round(E$values/sum(E$values),digits=3)\n")
            fo.write("pcaRes = list(x=X,eig=E,a=A,P=P,val=valprop)\n")
            fo.write("val = pcaRes$val\n")
            fo.write("nval = length(val)\n")
            if pictureFormat == "png":
                if bgcolor is None:
                    fo.write('png("%s.png",width=480,height=480\n'\
                         % (plotFileName) )
                else:
                    fo.write('png("%s.png",width=480,height=480,bg="%s")\n'\
                         % (plotFileName,bgcolor) )
            elif pictureFormat == "jpeg":
                if bgcolor is None:
                    fo.write('jpeg("%s.jpg",width=480,height=480\n'\
                         % (plotFileName) )
                else:
                    fo.write('jpeg("%s.png",width=480,height=480,bg="%s")\n'\
                         % (plotFileName,bgcolor) )
            elif pictureFormat == "xfig":
                if bgcolor is None:
                    fo.write('xfig("%s.fig",width=480,height=480\n'\
                         % (plotFileName) )
                else:
                    fo.write('xfig("%s.figg",width=480,height=480,bg="%s")\n'\
                         % (plotFileName,bgcolor) )
            elif pictureFormat == "pdf":
                if bgcolor is None:
                    fo.write('pdf("%s.pdf",width=6,height=6,title="PCA of relation valuation")\n' % (plotFileName) )
                else:
                    fo.write('pdf("%s.pdf",width=6,height=6,bg="%s",title="PCA of relation valuation")\n' % (plotFileName,bgcolor) )
            else:
                print('Error: Plotting device %s not defined !' % (pictureFormat))
                return
            ...
            fo.write('title(sub=paste("Total inertia:",(val[1]+val[2])*100,"%"),main="Factors 1 and 2",col.sub="black")\n')
            ...
            fo.write('title(sub=paste("Total inertia:",(val[3]+val[2])*100,"%"),main="Factors 3 and 2",col.sub="black")\n')        
            fo.write('abline(h=0,v=0,lty=2,col="gray")\n')
            ...
            fo.write('title(sub=paste("Total inertia:",(val[1]+val[3])*100,"%"),main="Factors 1 and 3",col.sub="black")\n')        
            fo.write('abline(h=0,v=0,lty=2,col="gray")\n')
        ...
        time.sleep(3)     
        if pictureFormat is not None and Comments:
            print('See %s.%s ! ' % (plotFileName,pictureFormat))
    ...
    def exportGraphViz(self,fileName=None,actionsSubset=None,
                       ...
                       pictureFormat=None,graphSize='7,7',
                       relation=None,bgcolor='cornsilk'):
        """
        ...
        for i in range(n):
            try:
                ...
                try:
                    nodeName = self.actions[actionkeys[i]]['name']
                except:
...
##             i += 1
    ...
    def _saveXML(self,fileName='temp',category='general',subcategory='general',author='digraphs Module (RB)',reference='saved from Python'):
        """
        save digraph in XML format. obsolete and deprecated.
        """
    ...
    def _saveXMCDA(self,fileName='temp',relationName='R',category='random',subcategory='valued',author='digraphs Module (RB)',reference='saved from Python',valuationType='standard',servingD3=False):
        """
        save digraph in XMCDA format. Obsolete and deprecated.
        """
        print('*----- saving digraph in XML format  -------------*')
        actions = [x for x in self.actions]
    ...
    def showStatistics(self):
        ...
        self.digraphDiameter = self.computeDiameter()
...
##        deter /= order * (order-1) * (Max - Med)
    ...
    def computeDiameter(self, Oriented = True):
        """
        Renders the (by default oriented) diameter of the digraph instance
        """
        order = len(self.actions)
        nbDepths = self.neighbourhoodDepthDistribution(Oriented=Oriented)
        nbDepths.reverse()
        if nbDepths[0] != 0:
            diameter = 'infinity'
        else:
...
##                    nb.add(a)
    ...
    def recodeValuation(self,newMin=-1.0,newMax=1.0,ndigits=4,Debug=False):
        """
...
##    def saveDistance2MaxCSV(self,fileName='tempdigraph',Normalized=False,
##                Dual=False,Converse=False,Diagonal=True,Debug=False):
##        """Persistent storage of a Digraph class instance in the form of
##            a csv file. """
##        from copy import deepcopy
##        import csv
...
##            for y in actionsList:
##                if x == y:
##                    if not Diagonal: # minimal distance
##                        rowText.append(0.0)
##                    else:
##                        rowText.append(float(Max - relation[x][y]))
##                else:
##                    rowText.append(float(Max - relation[x][y]))
##            if Debug:
##                print(rowText)
...
##        fo.close()
    ...
    def computeShortestPathLengths(self,WeakPaths=False,
                                   Comments=False,Debug=False):
        """
        ...
        *Source*: Claude Berge, *The Theory of Graphs*, Dover (2001) pp. 119, original in French Dunod (1958)
                ...
                for x in actions:
                    ...
                    for y in actions:
                        ...
                        if x != y:
                            ...
                            for z in actions:
                                #if x != z and y != z:
                                accz = max(accz,min(ri[i-1][x][z],ri[1][z][y]))
                            ri[i][x][y] = accz
        distances = {}
        for x in actions:
            distances[x] = {}
        ...
        if Comments:
            if WeakPaths:
                print('Shortest weak path lengths')
            else:
                print('Shortest path lengths')
            print('x, y, distance')
            for x in actions:
                for y in actions:
                    print(x,y,distances[x][y])
        if Debug:
            self.ri = ri
        if WeakPaths:
            self.shortestWeakPathLengths = distances
        else:
            self.shortestPathLengths = distances
        return distances

    #-------------------

    def computeDigraphCentres(self,WeakDistances=False,Comments=False):
        """
        ...
        *Source*: Claude Berge, *The Theory of Graphs*, Dover (2001) pp. 119, original in French Dunod (1958)
        ...
        if WeakDistances:
            try:
                distances = self.shortesWeakPathLengths()
            except:
                distances = self.computeShortestPathLengths(WeakPaths=True)
        else:
            try:
                distances = self.shortesPathLengths()
            except:
                distances = self.computeShortestPathLengths()
        ...
        for x in actions:
            ...
            for y in actions:
                if distances[x][y] is not None:
                    aN[x] = max(aN[x],distances[x][y])
                else:
                    aN[x] = order
        if Comments:
            if WeakDistances:
                print('Maximal weak neighborhood distances')
            else:
                print('Maximal neighborhood distances')
            for x in actions:
                if aN[x] < order:
                    print(x,aN[x])
                else:
                    print(x,None)
        self.maximalNeighborhoodDistances = aN
        minAN = order
        maxAN = 0
        for x in actions:
            minAN = min(minAN,aN[x])
            maxAN = max(maxAN,aN[x])
        # raduis and diameter    
        if minAN < order:
            self.radius = minAN
        else:
        ...
        if WeakDistances:
            if Comments:
                print('Digraph weak centre(s)', centres)
            self.weakCentres = centres
        else:
            if Comments:
                print('Digraph centre(s)', centres)
...
#------------------
    ...
    def showChordlessCircuits(self,Recompute=False):
        """
        ...
        """
        if Recompute:
            print('Recomputing the chordless circuits list.')
            self.computeChordlessCircuits()
...
#############
    ...
    def showHTMLBestChoiceRecommendation(self,pageTitle=None,
        ...
        if htmlFileName == None:
            ...
            fileName = getcwd()+'/'+htmlFileName
        fo = open(fileName,'w')
        fo.write(self._htmlBestChoiceRecommendation(
            pageTitle=pageTitle,
            ChoiceVector=ChoiceVector,
            CoDual=CoDual,
            Debug=Debug,
    ...
    def _htmlBestChoiceRecommendation(self,pageTitle=None,
                                          ChoiceVector=False,
                                          ContentCentered=True,
                                          CoDual=True,
                                          Debug=False,
            ...
            if gch[0] >= Med:
                ...
                for bch in g.badChoices:
                    if gch[5] == bch[5]:
                        ...
                        if gch[3] == gch[4]:
                            if Debug:
                                print(' === >> ambigous (first) choice ')
                                g.showChoiceVector(gch,choiceType='good',
                                                      ChoiceVector=ChoiceVector)
                                g.showChoiceVector(bch,choiceType='bad',
                                                      ChoiceVector=ChoiceVector)
                            html += '<h3>Ambiguous (first) choice(s)</h3>\n'
                ...
                if goodChoice:
                    if Debug:
                        print(' === >> potential first choice(s)')
                    html += '<h3>Potential first choice(s)</h3>\n'
                ...
                if Debug:
                    print(' === >> non robust first choice(s)')
                html += '<h3>Non robust first choice(s)</h3>\n'
                html += g.htmlChoiceVector(gch,choiceType='good',ChoiceVector=ChoiceVector)
        for bch in g.badChoices:
            if bch[0] >= Med:
                badChoice = True
                ...
                if Debug:
                    print('=== >> non robust last choice(s)')
                html += '<h3>Non robust lasst choice(s)</h3>\n'
                html += g.htmlChoiceVector(bch,choiceType='bad',ChoiceVector=ChoiceVector)
        if Debug:
            print()
            print('Execution time: %.3f seconds' % (t1-t0))
...
##        cover = temp.averageCoveringIndex(ker)
        ...
        goodChoiceVector.sort(reverse=True,key=itemgetter(0))
        if Comments:
    ...
    def computeKernelVector(self,kernel,Initial=True,Comments=False):
        ...
        choiceVector.sort(reverse=True,key=itemgetter(0))
        if Comments:
            if Initial:
    ...
    def htmlChoiceVector(self,ch,ChoiceVector=True,choiceType="good"):
        ...
        choice.sort()
        vec = ch[6]
        vec.sort(reverse=True, key=itemgetter(0))
...
##            print(netFlows)
    ...
    def _computeCopelandRanking(self):
        ...
        decCopelandScores.sort(key=itemgetter(0),reverse=True)
...
#                            Comments=False,Debug=False):
#         """
#         Renders a ranking from best to worst of the actions with maximal marginal correlation  and minimal marginal correlation amplitude.
...
##                        kcurr += relation[a[i]][a[j]] - relation[a[j]][a[i]]
    ...
    def computePrincipalScores(self, plotFileName=None,
        ...
        principalScores.sort(reverse=True,key=itemgetter(0))
        if Debug:
...
## respecting a maximum of marginal majority margins
    ...
    def computeSlaterOrder(self,isProbabilistic=False,
                           seed=None,sampleSize=1000,Debug=False):
        """
    ...
    def computeValuedRankingRelation(self,ranking):
        """
        ...
        """
        Med = self.valuationdomain['med']
        valuedRankingRelation = {}
        actionsList = [x for x in self.actions]
        for x in actionsList:
            valuedRankingRelation[x] = {}
            vrrx = valuedRankingRelation[x]
            xi = ranking.index(x)
            for y in actionsList:
                yj = ranking.index(y)
                if xi < yj:
                    vrrx[y] = max(Med,self.relation[x][y])
                elif xi == yj:
                    vrrx[y] = Med
                else:
                    vrrx[y] = min(Med,self.relation[x][y])
        return valuedRankingRelation
...
class IndeterminateDigraph(Digraph):
    ...
    """
    def __init__(self,other=None,nodes=None,
                 order=5,valuationdomain = (-1,1)):
        import sys,array,copy
        from collections import OrderedDict
...
class CirculantDigraph(Digraph):
    ...
    def __init__(self,order=7,
                 valuationdomain = {'min':Decimal('-1.0'),'max':Decimal('1.0')},
...
#         from copy import copy,deepcopy
#         if digraph is None:
#             from randomDigraphs import RandomValuationDigraphs
#             digraph = RandomValuationDigraph()
...
class BipartitePartialDigraph(Digraph):
    ...
    """
    def __init__(self,digraph,partA,partB,Partial=True):
        from copy import deepcopy
        self.valuationdomain = deepcopy(digraph.valuationdomain)
        self.name = 'bipartite_' + str(digraph.name)
        self.actions = deepcopy(digraph.actions)
        self.actionsA = partA
        self.actionsB = partB
        self.relation = self._constructRelation(digraph.relation,partA,partB,Partial)
        self.order = len(self.actions)
        self.gamma = self.gammaSets()
        self.notGamma = self.notGammaSets()

    def _constructRelation(self,relationIn,partA,partB,Partial):
        """
        Returns the asymmetric part of the relationIn
        """
        #actions = self.actions
        ...
        Min = self.valuationdomain['min']
        actions = self.actions
        # initialize relationOut to all  Med characteristics
        relationOut = {}
        for a in actions:
            relationOut[a] = {}
            for b in actions:
        ...
        for b in partB:
            for a in partA:
                relationOut[b][a] = relationIn[b][a]
        if not Partial:
            for a in partA:
                for b in partA:
                    if a != b:
...
class CriteriaCorrelationDigraph(Digraph):
    ...
    def __init__(self,outrankingDigraph,
                 ValuedCorrelation=True,
                 WithMedian=False):
        from copy import deepcopy
        from collections import OrderedDict
        if ValuedCorrelation:
            self.name = outrankingDigraph.name + 'valCorr'
        else:
            self.name = outrankingDigraph.name + 'corr'
        actions = OrderedDict()
        for g in outrankingDigraph.criteria:
            godg = outrankingDigraph.criteria[g]
            actions[str(g)] = {'shortName':str(g), 'name':godg['name']}
        if WithMedian:
            actions['m'] = {'shortName':'m', 'name':'outranking relation'}
        valuationdomain = {'min':Decimal('-1.0'),'med':Decimal('0.0'),'max':Decimal('1.0')}
        Min = valuationdomain['min']
        Med = valuationdomain['med']
        Max = valuationdomain['max']
        criteriaList = [g for g in outrankingDigraph.criteria]
        n = len(criteriaList)
        corr,d = outrankingDigraph.computeCriteriaCorrelations(\
            ValuedCorrelation=ValuedCorrelation)
        relation = {}
        for i in range(n):
            relation[criteriaList[i]] = {}
            for j in range(n):
                relation[criteriaList[i]][criteriaList[j]] =\
                                corr[criteriaList[i]][criteriaList[j]]
        if WithMedian:
            margCorr = outrankingDigraph.computeOutrankingConsensusQuality(\
                ValuedCorrelation=ValuedCorrelation)
            #print(margCorr)
            relation['m'] = {}
            relation['m']['m'] = outrankingDigraph.computeDeterminateness()
    ...
    def _saveCorrelationDistancesCSV(self,fileName='tempdigraph',Normalized=False,
                Dual=False,Converse=False,Diagonal=True,Debug=False):
        """ file exchange with R for the principal image drawing"""
        from copy import deepcopy
        import csv
        com = ''
        if Normalized:
    ...
    def exportPrincipalImage(self, plotFileName=None,
                             pictureFormat="pdf",
                             bgcolor='cornsilk',
                             fontcolor='red3',
                             fontsize='0.85',
        ...
        if plotFileName is None:
            ...
            plotFileName = "%s/%s" % (tempDir,plotFileName)
        self._saveCorrelationDistancesCSV('%s/exportTemp' % tempDir,Diagonal=True)
        #if _Colwise:
        #fo = open('%s/tempCol.r' % tempDir,'w')
        #else:
        fo = open('%s/tempRow.r' % tempDir,'w')
...
if __name__ == "__main__":
    ...
    # partA = ['a1','a2']
    # partB = ['a3','a4','a5']
    # bpg = BipartitePartialDigraph(g,partA,partB)
    # bpg.showRelationTable()
    # bpg1 = BipartitePartialDigraph(g,partA,partB,Partial=False)
    # bpg1.showRelationTable()
    
##    gcd = ~(-g)
##    #g = RandomValuationDigraph()
##    Digraph.showRelationTable(g,rankingRule='Copeland')
##    g.recodeValuation(ndigits=4)
##    print(g.relation)
##    print(g.isOutrankingDigraph(Debug=False))
##    gcd.showRelationTable(rankingRule='NetFlows')
##    gcd.recodeValuation(ndigits=None)
##    print(gcd.relation)
##    print(gcd.isStrictOutrankingDigraph(Debug=False))
##    
    # g.recodeValuation(0,1)
    # g.showRelationTable()
    # print(g.isOutrankingDigraph(Debug=False))
    # print((-g).isStrictOutrankingDigraph(Debug=False))
    
    #g.showChordlessCircuits(Recompute=True)
    #g.isAsymmetricIndeterminate(Debug=True)
    #gc = BrokenChordlessCircuitsDigraph(g)

    #gcoc = BrokenCocsDigraph(g)
...
##    cc = g.computeCriteriaCorrelationDigraph(ValuedCorrelation=True,WithMedian=True)
    ...
    #Digraph.exportPrincipalImage(cc,'testa',pictureFormat='pdf',fontsize='0.75',Comments=True)
    #g.recodeValuation(0,2)
    #g.exportPrincipalImage('testd',Reduced=True,pictureFormat='pdf',fontsize='0.75',Comments=True)
    #g.exportPrincipalImage('testc',Colwise=True,pictureFormat='pdf',fontsize='0.75',Comments=True)
    
    #g.exportGraphViz(pictureFormat='png')
##    print(g)
##    g.showAttributes()
